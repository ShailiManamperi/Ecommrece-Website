...
// add site item details 
router.post("/item-details", upload.single("image"), async (req, res) => {
    try {
        const { I_Id, stDesc, lgdesc, width, height, depth, mnCategory,subCategory,brand, type } = req.body;
        const image = req.file ? req.file.buffer : null;

        // Validation
        if (!I_Id || !stDesc || !lgdesc) {
            return res.status(400).json({
                success: false,
                message: "Required fields missing (I_Id, stDesc, lgdesc are mandatory)",
            });
        }

        // Check if item exists in Item table
        const [itemExists] = await db.query("SELECT I_Id FROM item WHERE I_Id = ?", [I_Id]);
        if (itemExists.length === 0) {
            return res.status(400).json({
                success: false,
                message: "Invalid I_Id — no matching Item found.",
            });
        }

        // ✅ Update Item table with mnCategory and type
        await db.query(
            "UPDATE item SET mnCategory = ?, type = ?,subCategory =? , brand=? WHERE I_Id = ?",
            [mnCategory || null, type || null,subCategory|| null,brand || null, I_Id]
        );

        // ✅ Check if record already exists in item_details
        const [existingDetails] = await db.query(
            "SELECT * FROM item_details WHERE I_Id = ?",
            [I_Id]
        );
        ...
        if (existingDetails.length > 0) {
            return res.status(200).json({
                success: true,
                message: "Item details already exist, no new record added.",
                data: existingDetails[0],
            });
        }

        // ✅ Insert record into item_details
        await db.query(
            `INSERT INTO item_details (I_Id, stDesc, lgdesc, width, height, depth, image)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [I_Id, stDesc, lgdesc, width || null, height || null, depth || null, image || null]
        );

        return res.status(201).json({
            success: true,
            message: "Item details added successfully and Item table updated",
            data: {
                I_Id,
                stDesc,
                lgdesc,
        ...
        return res.status(500).json({
            success: false,
            message: "Error adding item details",
            details: err.message,
        });
    }
});
...
router.get("/itemdetails", async (req, res) => {
    try {
        // Join item_details with Item to get sell price and item name
        const [rows] = await db.query(`
            SELECT 
                ...
                i.I_name
            FROM item_details d
            JOIN item i ON d.I_Id = i.I_Id
        `);

        // Convert image buffer to base64
        const data = rows.map((row) => ({
            I_Id: row.I_Id,
            I_name: row.I_name || "No Name",
            stDesc: row.stDesc,
            lgdesc: row.lgdesc,
            width: row.width,
            height: row.height,
            depth: row.depth,
            image: row.detailImage ? row.detailImage.toString("base64") : null,
            sellPrice: row.sellPrice
        }));

        return res.status(200).json({
            success: true,
            message: "Fetched all item details with sell price and name",
            data
        });
...
// pass for shop categroy

router.get("/itemdetails-categoryvice", async (req, res) => {
  const { category, subCategory, brand, color } = req.query;
  ...
  try {
    ...
    const data = rows.map(row => ({
      ...
      category: row.mnCategory || "Uncategorized",
...
router.get("/itemdetails/:id", async (req, res) => {
    try {
        ...
        // Query join Item and item_details
        const [rows] = await db.query(`
            SELECT 
                i.I_Id,
                i.I_name,
                ...
                d.image AS detailImage
            FROM item i
            LEFT JOIN item_details d ON i.I_Id = d.I_Id
            WHERE i.I_Id = ?
        `, [id]);
        ...
        const result = {
            ...
            height: item.height,
            depth: item.depth,
            image: item.detailImage ? item.detailImage.toString("base64") : null
        };
...
router.get("/allitems", async (req, res) => {
    try {
        ...
        const [items] = await db.query(`
            SELECT 
                i.I_Id,i.I_name,i.descrip,i.material,i.price,i.type,i.stockQty,i.availableQty,i.mnCategory,i.type,
                i.reservedQty,i.bookedQty,i.warrantyPeriod,i.color,o.OrID,o.billnumber,i.brand,i.subCategory,
...
router.post("/delivery-payment", async (req, res) => {
    ...
    try {
        ...
        const [driverData] = await db.query("SELECT balance FROM D\driver WHERE devID = ?", [driverId]);
